...
class MotionLib():
    def __init__(
        ...
        print("using motion lib v2...")
        self.device = device
        self.key_body_ids = torch.tensor(key_body_ids, device=device)
        ...
        self.keypoint_trans = torch.cat([motion['keypoint_trans'] for motion in motions],   # (total_frames, 12, 3)
                                        dim=0).float().to(self.device)
        self.lbp = torch.cat([motion['local_key_body_pos'] for motion in motions],   # (total_frames, 12, 3)
                                        dim=0).float().to(self.device)
    ...
    def sample_time(self, motion_ids, truncate_time=None):
        ...
        motion_ids = motion_ids.to(self.motion_lengths.device)
    ...
    def get_motion_num_frames(self, motion_ids):
        motion_ids = motion_ids.to(self.motion_num_frames.device)
        return self.motion_num_frames[motion_ids]
    
    def get_motion_fps(self, motion_ids):
        motion_ids = motion_ids.to(self.motion_fps.device)
    ...
    def get_motion_difficulty(self, motion_ids):
        ...
        motion_ids = motion_ids.to(motions_difficulty_tensor.device)
        return motions_difficulty_tensor[motion_ids]
