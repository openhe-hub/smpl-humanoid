...
class H1Mimic(LeggedRobot):
    ...
    def _load_motion(self, motion_file, no_keybody=False):
        ...
        self._motion_lib = MotionLib(
            motion_cfg_path=motion_file,
            motion_folder_path=r'/home/openhe/Workspace/program/python/expressive-humanoid/ASE/ase/poselib/data/retarget_npy/amass/female_walking',
            device=self.device,
            key_body_ids=self._key_body_ids.cpu().numpy(),
        )
        return
    ...
    def reset_idx(self, env_ids, init=False):
        ...
        if self.cfg.motion.motion_curriculum:
            ...
            self._motion_lengths = self._motion_lengths.to(self.episode_length_buf.device)
            env_ids = env_ids.to(self.episode_length_buf.device)
        ...
        # print(f"dof_pos => {dof_pos_motion.shape}")
        # print(f"root_vel => {root_vel.shape}")
        # print(f"root_ang_vel => {root_ang_vel.shape}")
